...
class ActorPPOTrainer(BasePPOTrainer):
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                for i, experience in enumerate(
                    self.make_experience_maker().make_experience_list(rand_prompts, **self.generate_kwargs)
                ):
                    if i == 0:
    ...
    def evaluate(self, eval_dataloader, global_step, n_samples_per_prompt=1, eval_steps=1):
        ...
        if self.strategy.ring_attn_group is None or self.strategy.ring_attn_rank == 0:
            ...
            with torch.no_grad():
                ...
                samples = self.make_experience_maker().generate_samples(all_prompts, **generate_kwargs)
                ...
                if self.strategy.ring_attn_group is not None:
                    ...
                    torch.distributed.all_gather_object(
                        gathered_metrics, local_metrics, group=self.make_experience_maker().ring_rank0_group
                    )
                else:
    ...
    def make_experience_maker(self):
...
@ray.remote()
def make_experience_list_remote(
    actor,
    critic,
    reward_model,
    initial_model,
    ...
    exp_maker = RemoteExperienceMaker(
        actor=actor,
        critic=critic,
    ...
    return exp_maker.make_experience_list(prompts, **generate_kwargs)